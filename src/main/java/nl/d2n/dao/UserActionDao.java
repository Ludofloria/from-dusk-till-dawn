...
public class UserActionDao {
    ...
    public List<UserAction> findActions(Long cityId, Long userId) {
        return (List<UserAction>)entityManager
                .createQuery("from UserAction ua where ua.city.id = :city_id and ua.user.id = :user_id", UserAction.class)
    ...
    */
    public List<LastUpdateByUser> findLastUpdateTimes(Long cityId) {
    ...
    @SuppressWarnings({"unchecked"})
    public List<UserUpdateCount> findRecentChanges(Long cityId, Date startDate, Date endDate) {
        ...
        for (Object result : results) {
            ...
            counts.add(new UserUpdateCount(getLongValue(resultArray[0]), getIntValue(resultArray[1]), getIntValue(resultArray[2])));
        }
        return counts;
    }

    protected Long getLongValue(Object object) {
        return object instanceof BigDecimal ? ((BigDecimal)object).longValue() : ((BigInteger)object).longValue();
    }
