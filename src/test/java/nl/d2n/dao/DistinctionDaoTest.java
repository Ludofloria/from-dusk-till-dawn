...
public class DistinctionDaoTest extends SpringContextTestCase {
    ...
    public void testDistinctionFetch() {
        User user1 = classCreator.createUser("Bob",  new UserKey("cafebabe1"), 11L);
        User user2 = classCreator.createUser("Dale", new UserKey("cafebabe2"), 22L);
        ...
        Map<Long, UserWithProfile> profiles =
                distinctionDao.findUsersWithDistinctions(1984L, userDao.findUserIds(UserCitizenConverter.convertUsersToCitizens(new User[] { user1, user2 })));
    ...
    public void getTopUsers() {
        User user1 = classCreator.createUser("Bob",  new UserKey("cafebabe1"), 11L);
        User user2 = classCreator.createUser("Dale", new UserKey("cafebabe2"), 22L);
        User user3 = classCreator.createUser("Anne", new UserKey("cafebabe3"), 33L);
        ...
        List<UserWithDistinction> users = distinctionDao.findTopUsersWithDistinction(1L, 2);
    ...
    public void testDistinctionFetchForZeroCitizens() {
        try {
            Map<Long, UserWithProfile> profiles = distinctionDao.findUsersWithDistinctions(1984L, new ArrayList<Long>());
        } catch (Exception err) {
