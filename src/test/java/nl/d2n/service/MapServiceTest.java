...
public class MapServiceTest extends SpringContextTestCase {
    ...
    public void updateItemsManually() throws Exception {
        ...
        List<Zone> zones = zoneDao.findZones(14390L);
        ...
        zones = zoneDao.findZones(14390L);
    ...
    public void updateZoneForTheFirstTime() throws Exception  {
        ...
        assertEquals(1, zoneDao.findZones(14390L).size());
    }
    ...
    public void getZones() throws Exception {
        cache.setD2NXmlFile(new D2NXmlFile(D2NXmlReaderFromFile.INPUT_PATH));
        City city = classCreator.createCity(13449L, "Highlanders Inc");
        Map<Integer, Map<Integer, Zone>> zones = mapService.getZones(13449L);
        assertEquals(27, zones.keySet().size());
    }

    @Test
    public void getBuildingsWithStatus() throws JAXBException {
        uniqueInsideBuildingDao.save(new UniqueInsideBuilding(1042L, "Upgraded Map", false, 1050L, "item_electro", "Lorem ipsum", "http://somewiki.com/upgraded_map", false, false));
        uniqueInsideBuildingDao.save(new UniqueInsideBuilding(1050L, "Watchtower",   false, null, "item_tagger", "Lorem ipsum", null, false, false));
        uniqueInsideBuildingDao.save(new UniqueInsideBuilding(1062L, "Portal Lock",   true, null, "small_door_closed", "Lorem ipsum", null, false, false));
        uniqueInsideBuildingManager.refresh();
        classCreator.createInsideBuilding(classCreator.createCity(1666L, "Cliffe of tricks"), 1042L, InsideBuildingStatus.AVAILABLE);
        String xml =
        ...
        List<InsideBuilding> buildings = mapService.getBuildingsWithStatus(city.getBuildings(), 1666L);
        assertInsideBuilding(buildings.get(0), 1050L, InsideBuildingStatus.CONSTRUCTED);
        assertInsideBuilding(buildings.get(0).getChildBuildings().get(0), 1042L, InsideBuildingStatus.AVAILABLE);
        assertInsideBuilding(buildings.get(1), 1062L, InsideBuildingStatus.NOT_AVAILABLE);
    }

    @Test
    public void checkAlwaysAvailableBuildings() throws JAXBException {
        uniqueInsideBuildingDao.save(new UniqueInsideBuilding(1042L, "Upgraded Map", false, null, "item_electro", "Lorem ipsum", "http://somewiki.com/upgraded_map", false, true));
        uniqueInsideBuildingManager.refresh();
        String xml =
            "<city city=\"Cliffe of tricks\" door=\"1\" water=\"0\" chaos=\"0\" devast=\"0\" x=\"16\" y=\"11\">" +
            "</city>";
    ...
    public void userNotInGame() throws ApplicationException {
        User user = classCreator.createUser("Heltharion", new UserKey("abcdef"), true, false, false, classCreator.createCity(13449L, "Cliffe of tricks"));
    ...
    public void userInTown() throws ApplicationException {
        City city = classCreator.createCity(13449L, "Cliffe of tricks");
        ...
        assertTrue("Zone 1/1 must be depleted", zoneDao.findZone(13449L, 1, 1).isZoneDepleted());
    }
    ...
    protected void assertInsideBuilding(InsideBuilding building, Long id, InsideBuildingStatus status) {
